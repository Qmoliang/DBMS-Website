...
$bsql = "with s1 as (select extract(month from date_sales) as month, extract(year from date_sales) as year, product_ID as productID, c_id as userID, price
            FROM sale
            where date_sales between to_date('".$_GET['p1']."', 'yyyy-mm') and to_date('".$_GET['p2']."', 'yyyy-mm')),
     s2 as (select s1.* from s1 join users on userID = users.ID where area = '".$_GET['p3']."'),
     s3 as (select year, month, productID, sum(price) as SaleValue from s2 group by year, month, productID),
     s4 as (select year, month, (select name from products where id = productID), SaleValue from (select s3.*, row_number() over(partition by s3.year, s3.month order by s3.SaleValue DESC) RN from s3) WHERE RN <= 10),
     s5 as (select distinct year,month from s1 order by year,month),
     s6 as (select count(*) as chartnum from s5)
select * from s4,s6";
/*		
...
$p_line_num=1;
$p_chart_num=$results['CHARTNUM'][0];
$canshuqueren=array(0,0,0);
$duiyingxian=array(0,0,0);

$yisao=0;

for ($i = 0; $i < $p_chart_num;$i++)
{
	
	$yuefen=$results['YEAR'][$yisao]."-".$results['MONTH'][$yisao];
	
	for($j1=0;$j1<10;$j1++)
	{
		if( ($results['YEAR'][$yisao]."-".$results['MONTH'][$yisao]) == $yuefen)
		{
		$p_datasets_0_label[$i][0] =  "Sale value of Top 10 products in ".$_GET['p3']."-".$results['YEAR'][$yisao]."-".$results['MONTH'][$yisao]; 
		$p_labels[$i][]=$results['(SELECTNAMEFROMPRODUCTSWHEREID=PRODUCTID)'][$yisao];
		$p_datasets_0_data[$i][0][]=$results['SALEVALUE'][$yisao];
		$yisao++;
		}
	}
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="jumbotron">
                        ...
                        <p> Show the top 10 product changes by monthly sales value in a certain period of time
<p>User options: time range, area
<p>Application scenarios: assist companies to analyze product advantages, and select major products for further promotion. </p>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="jumbotron">
                        <h4>Selection</h4>
                        <form action="6th_query.php"  class="form-inline">
                ...
                <div class="col-md-12">
                    <div class="jumbotron">
                        <div class="panel panel-default chartJs">
							...
							for ($i3= 0; $i3< $p_chart_num;$i3++)
							{
								echo '<div class="panel-body" >
                                <canvas id="line-chart'.$i3.'" class="chart" style="position: relative; height:100vh; width:80vw"></canvas>
                            </div><br><br><br>';

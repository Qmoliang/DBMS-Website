...
$bsql ="with t1 as (SELECT extract(month from date_sales) as month, extract(year from date_sales) as year, price, product_ID as productID, c_id as userID, qty FROM sale
            where date_sales between to_date('".$_GET['p1']."', 'yyyy-mm') and to_date('".$_GET['p2']."', 'yyyy-mm') and o_status = 'success'),
     t2 as (SELECT * from t1 join users on userID = users.ID where area = '".$_GET['p3']."')
SELECT '".$_GET['p3']."' as area, year, month, sum(price) as salesvalue, sum(qty) as salevolume from t2 group by year, month  order by year, month";
...
for ($i = 0; $i < count($results['YEAR']);$i++)
      {
		   ...
		   if($results['AREA'][$i]==$_GET['p3'])
		   {
			   ...
			   $p_datasets_0_data[$nchart][$duiyingxian[0]][]=$results['SALESVALUE'][$i];
				   }
				   if($nchart==1)
				   {
			   //$p_labels[$nchart][]=$results['YEAR'][$i]."-".$results['MONTH'][$i];
    ...
    <div id="page-wrapper">
        ...
        <div id="page-inner">
            <div class="row">
                <div class="col-md-12">
                    <div class="jumbotron">
                        <h2>Regional distribution query</h2>
                        <p> Shows the trend of the regional market share by region over a period of time in each month. The sales value and sales volume are displayed separately.
<p>User options: time range, region
<p>Application scenario: Regional distribution query can help enterprises check the rationality of regional layout and consistency with the overall corporate strategy. </p>
                    </div>
                </div>
					<div class="col-md-12">
                    <div class="jumbotron">
                        <h4>Selection</h4>
                        <form action="2nd_query.php"  class="form-inline">
